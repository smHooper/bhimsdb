...
var BHIMSEntryForm = (function() {
	...
	*/
	Constructor.prototype.configureForm = function(mainParentID=null, isNewEntry=true) {
		...
		$.when(
			...
			// Set up the map only if it isn't already
			// if (!this.maps.main.mileposts.currentLayer) {
	...
	*/
	...
	// Constructor.prototype.setMapMileposts = function(mapObject, {forceReload=false}={}) {
	// 	const map = mapObject.map;
	// 	const mapZoom = map.getZoom();
	// 	const previousZoom = mapObject.previousZoom;
	// 	var zoomLevels = Object.keys(mapObject.mileposts.layers).map(l => parseInt(l));

	// 	// record new zoom level 
	// 	mapObject.previousZoom = mapZoom;

	// 	// Figure out if the mileposts need to be reloaded
	// 	// If we're at a milepost zoom step, we'll need to reload the layer
	// 	if (!forceReload && !(zoomLevels.includes(mapZoom) || zoomLevels.includes(previousZoom)) ) {
			
	// 		return;
	// 		// const sortedZooms = zoomLevels.concat([previousZoom, mapZoom]).sort();
	// 		// const previousZoomIndex = sortedZooms.indexOf(previousZoom);
	// 		// const currentZoomIndex = sortedZooms.indexOf(mapZoom);
	// 		// if ()
	// 	}

	// 	//const milepostGroups = mapObject.mileposts.layer;
	// 	const milepostLayer = mapObject.mileposts.layer;
	// 	// when form is loaded, milepostLayer will be null
	// 	if (milepostLayer) {
	// 		map.removeLayer(milepostLayer);
	// 		mapObject.layerControl.removeLayer(milepostLayer);
	// 	}
	...
	// 	// 	"features": []
	// 	// }
	// 	// for (const layerZoom in mapObject.mileposts.data) {
	// 	// 	const featureGroup = mapObject.mileposts.data[layerZoom];
	// 	// 	// If the zoom level is high enough, add the layer
	// 	// 	if (layerZoom < mapZoom) {
	// 	// 		milepostGeojson.features = [...milepostGeojson.features, ...featureGroup];
	// 	// 	}
	// 	// }
	// 	// const pointToLayer = (feature, latlon) => 
	// 	// 	L.circleMarker(latlon, markerOptions)
	// 	// 		.bindTooltip(
	// 	// 			layer => layer.feature.properties.mile.toString(), 
	// 	// 			{permanent: true, className: 'leaflet-tooltip-point-label'}
	// 	// 		);
	// 	// var layer = L.geoJSON(milepostGeojson, {pointToLayer: pointToLayer})
	// 	// 	.addTo(map);

		
	// 	// get the layer of the zoom level just below this one
	// 	zoomLevels.push(mapZoom);
	// 	zoomLevels.sort((a, b) => a - b);

	// 	const index = zoomLevels.indexOf(mapZoom) - 1;
	// 	const layer = mapObject.mileposts.layers[zoomLevels[index]];
	// 	const previousLayer = mapObject.mileposts.currentLayer
		
	// 	if (layer === previousLayer) return;

	// 	if (previousLayer) {
	// 		mapObject.map.removeLayer(previousLayer)
	// 		mapObject.layerControl.removeLayer(mapObject.mileposts.currentLayer);
	// 	}
	// 	if (layer) {		
	// 		mapObject.map.addLayer(layer);
	// 		mapObject.layerControl.addOverlay(layer, 'Mileposts');
	// 		mapObject.mileposts.currentLayer = layer;
	// 	}
	// 	//mapObject.mileposts.geojson = milepostGeojson;
	// }
	...
	*/
	...
	// 	mapObject.previousZoom = mapZoom || 7
	// 	var map = L.map(divID, {
	// 		editable: true,
	// 		scrollWheelZoom: false,
	// 		center: mapCenter || [63.2, -150.7],
	// 		zoom: mapObject.previousZoom
	// 	});
	...
	// 	mapObject.layerControl = layerControl;
	...
	// 		.done(geojson => {
	// 			mapObject.mileposts.geojson = geojson;
	...
	// 				if (mile == 0) continue; // skip milepost 0
	// 				// const layerGroup = 
	// 				// 	mile % 100 === 0 ? mapObject.mileposts.data[3] :
	// 				// 	mile % 50 === 0 ? mapObject.mileposts.data[5] :
	// 				// 	mile % 20 === 0 ? mapObject.mileposts.data[6] :
	// 				// 	mile % 10 === 0 ? mapObject.mileposts.data[9] :
	// 				// 	mile % 5 === 0 ? mapObject.mileposts.data[10] :
	// 				// 	mapObject.mileposts.data[11]; // everything else
	// 				// layerGroup.push(feature);
	// 				if (mile % 100 === 0) zoomGeojsons[3].features.push(feature);
	// 				if (mile % 50  === 0) zoomGeojsons[5].features.push(feature);
	// 				if (mile % 20  === 0) zoomGeojsons[7].features.push(feature);
	// 				if (mile % 10  === 0) zoomGeojsons[9].features.push(feature);
	// 				if (mile % 5   === 0) zoomGeojsons[10].features.push(feature);
	// 				zoomGeojsons[11].features.push(feature); // make sure there's one with everything
	// 			}
	// 			const markerOptions = {
	// 				radius: 3,
	// 				weight: 1,
	...
	// 						{permanent: true, className: 'leaflet-tooltip-point-label'}
	// 					);
	// 			for (const zoom in zoomGeojsons) {
	// 				mapObject.mileposts.layers[zoom] = L.geoJSON(zoomGeojsons[zoom], {pointToLayer: pointToLayer})
	// 			}
	...
	// 			this.setMapMileposts(mapObject, {forceReload: true});
